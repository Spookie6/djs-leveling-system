...
client.on(Events.InteractionCreate, async (interaction) => {
	...
	if (interaction.isCommand()) {
		...
		if (
			cmd.developerOnly &&
			!client.config.developers.includes(interaction.member.id)
		) {
			interaction
				.followUp(`This commands is developer only!`)
				.then((msg) => {
					setTimeout(() => {
						msg.delete();
					}, 5000);
		...
		if (!bitPermissions.has(cmd.permissions || [])) {
			interaction
				.followUp(`You don't have permission to use this command!`)
				.then((msg) => {
					setTimeout(() => {
						msg.delete();
					}, 5000);
		...
		if (
			client.cooldowns.has(
				`${cmd.name}${interaction.member.id}${interaction.guild.id}`
			)
		) {
			interaction
				.followUp(
